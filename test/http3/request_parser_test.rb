...
class RequestParserTest < Minitest::Test
  ...
  def test_parses_post_with_body
    ...
    data = build_frame(Quicsilver::Protocol::FRAME_HEADERS, headers_payload) + build_frame(Quicsilver::Protocol::FRAME_DATA, "body content")
  ...
  def test_to_rack_env
    ...
    body = "test body".b
    data = build_frame(Quicsilver::Protocol::FRAME_HEADERS, headers_payload) + build_frame(Quicsilver::Protocol::FRAME_DATA, body)
    parser = Quicsilver::Protocol::RequestParser.new(data)
    parser.parse
  ...
  def test_to_rack_env_returns_nil_when_no_headers
    assert_nil Quicsilver::Protocol::RequestParser.new("").tap(&:parse).to_rack_env
  end
  ...
  def test_parse_multiple_data_frames
    ...
    data += build_frame(Quicsilver::Protocol::FRAME_DATA, "chunk1")
    data += build_frame(Quicsilver::Protocol::FRAME_DATA, "chunk2")
    data += build_frame(Quicsilver::Protocol::FRAME_DATA, "chunk3")
  ...
  def test_indexed_field_with_empty_value
    ...
    frame = build_frame(Quicsilver::Protocol::FRAME_HEADERS, payload)

    parser = Quicsilver::Protocol::RequestParser.new(frame)
    parser.parse
  ...
  def test_rejects_max_push_id_frame_on_request_stream
    ...
    data = build_frame(Quicsilver::Protocol::FRAME_HEADERS, headers_payload)
    data += build_frame(Quicsilver::Protocol::FRAME_MAX_PUSH_ID, "\x00")
  ...
  def test_truncated_frame_length_does_not_hang
    ...
    parser = Quicsilver::Protocol::RequestParser.new(truncated)
  ...
  def test_rejects_data_before_headers
    data = build_frame(Quicsilver::Protocol::FRAME_DATA, "body first")
    data += build_frame(Quicsilver::Protocol::FRAME_HEADERS, build_qpack_headers(
  ...
  def test_frames_are_recorded
    ...
    body_data = "test"
    data = build_frame(Quicsilver::Protocol::FRAME_HEADERS, headers_payload)
    data += build_frame(Quicsilver::Protocol::FRAME_DATA, body_data)
  ...
  def build_frame(type, payload)
    frame_type = Quicsilver::Protocol.encode_varint(type)
    frame_length = Quicsilver::Protocol.encode_varint(payload.bytesize)
